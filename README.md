...
The principles are simple, articulate your process as a workflow and then begin to divide-and-conquer individual, discrete "tasks" within the workflow. A few iteration cycles and your workflow can be broken down into a simple "chain" of tasks.
...
The workflow relies on a "namespace" (NS) for each workflow you design and implement. We'll see later why this is. The NS provides a scoped contruct that the workflow will utilize to dynamically generate task class runtime instances. Inside this NS, you define the following :
